...
- **Генератор клиентского `.conf`** — читает параметры WG-интерфейса прямо с роутера (server public key, listen-port, подсеть, MTU, WAN-endpoint) и собирает готовый конфиг; приватный ключ не хранится
- **3 режима**: весь трафик через анти-DPI / выборочно по подсетям / обычный доступ к домашней LAN. Для full-tunnel — безопасная split-форма `AllowedIPs` (без литерального `0.0.0.0/0`, который взводит kill-switch WireGuard-for-Windows) + опция заворота IPv6 в туннель против утечки реального адреса
- **➕ Помощник «Новый клиент»** — генерация пары ключей + пошаговая инструкция, что создать в Keenetic (интерфейс, пир, правила Межсетевого экрана)
- **🔌 Подключение интерфейса к анти-DPI** — одной кнопкой ставит/снимает scoped `netfilter.d`-хук (заворот трафика интерфейса в xray) + cron self-heal; интерфейсы под внешними хуками защищены от случайного отключения
- Сводный список интерфейсов, заведённых в анти-DPI; копирование/скачивание `.conf`
