...
- ✅ **Git for Windows** (`git-scm.com/download/win`) — для установки и обновления одной командой (`update.bat`). Без Git панель тоже ставится — см. врезку **«Нет Git? Установка из ZIP»** ниже
...
> **Нет Git? Установка из ZIP (без `git clone`):**
> Git нужен только для обновления одной командой (`update.bat`). Сама панель работает и без него.
> 1. На странице репозитория нажми зелёную кнопку **`Code` → `Download ZIP`** (либо на вкладке **Releases** под нужной версией → **`Source code (zip)`**).
> 2. Распакуй архив. Внутри будет вложенная папка вида `xkeen-windows-dashboard-main` — перенеси её содержимое (или переименуй саму папку) в `C:\xray-dashboard`, чтобы `install.bat` лежал прямо как `C:\xray-dashboard\install.bat`.
> 3. Дальше всё как обычно: `.\install.bat` → правишь `config_local.py` → `.\start-bg.bat`.
> 4. ⚠ **Обновление без Git** делается вручную (`update.bat` использует `git pull` и не сработает) — пошагово в разделе **«Как обновлять»** → **«Без Git (из ZIP)»** ниже.
...
**Без Git (если ставил из ZIP):** `update.bat` не сработает — он использует `git pull`. Обновляйся вручную:
...
Минус ручного способа: если в новой версии какой-то файл удалили, при распаковке поверх он останется лишним (безвредно). Чтобы обновления были в одну команду — поставь Git и сделай `git clone` заново.
