...
- ✅ **Windows-PC** (10 или 11). Linux/macOS пока не тестировался
...
- **Подписки VPN**: добавить subscription URL → панель скачивает список серверов с провайдера, выкладывает их как outbounds на роутер. Группировка по подписке, отображение даты истечения с цветной шкалой, ручное обновление кнопкой
...
Если XKeen уже работает (трафик идёт через VPN) — переходи к шагу 2.
...
### Шаг 11. Проверить что VPN работает

С устройства которое привязал к политике `XKeen`:

1. Открой **новый таб инкогнито** в браузере
2. Зайди на https://checkip.amazonaws.com/ (НЕ 2ip.ru — он показывает RU-IP даже когда VPN работает, потому что .ru-домены идут через провайдера напрямую по дизайну)
...
**Видишь Mixed-режим в `xkeen -status`** — это **валидный режим работы** XKeen (не ошибка). VPN работает. Проверь через `checkip.amazonaws.com` (не 2ip.ru).
...
## Сценарий «работа/гость» — всё мимо VPN, через VPN только YouTube/AI
...
**🌐 Напрямую без VPN — через провайдера** (первая опция, серо-оранжевая группа).
В AI и YT поставь VPN-каналы как обычно. Готово: рабочий/гостевой/банковский трафик
идёт напрямую, а YouTube без рекламы и AI без геоблока — через VPN.
...
- `sub_expiry_notifier.py` — опциональный нотифайер о истечении VPN-подписок (для Task Scheduler)
...
MIT — см. [LICENSE](LICENSE). Используй свободно, модифицируй, форкай, применяй в коммерческих проектах.

### ⭐ Если форкаешь или используешь в своём проекте

По MIT-лицензии **обязательно** сохранить copyright notice из [LICENSE](LICENSE) в копиях/форках.

Дополнительно — буду благодарен за **упоминание оригинала** в README твоего проекта:

```
Based on [xkeen-windows-dashboard](https://github.com/yuran2000/xkeen-windows-dashboard)
```

или просто ссылка-звёздочка [⭐ Star](https://github.com/yuran2000/xkeen-windows-dashboard) — это не правовое, а этическое обязательство, но именно так растёт open-source community.
